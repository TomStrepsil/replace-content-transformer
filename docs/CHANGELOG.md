...
- Added branch comparison and PR summary creation scripts to the benchmarking workspace
...
- Removed jsdoc comment on `buffered-indexOf-anchored` benchmarking strategy
- Added missing `number` typing to indices in typescript-formatted JSDoc comments

### Removed

- Removed the `bench` alias for the `bench:node` script in the `test/benchmarks` workspace package
