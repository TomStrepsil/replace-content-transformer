 ...
 * - `{ isMatch: true, content: T, streamIndices: [startIndex: number, endIndex: number] }` - Match value passed to replacement function (endIndex is exclusive / half-open interval)
 *
...
export interface SearchStrategy<TState, TMatch = string> {
   ...
   * @yields MatchResult - Either `{ isMatch: false, content: string }` or `{ isMatch: true, content: TMatch, streamIndices: [startIndex: number, endIndex: number] }`
   */
  processChunk(
    haystack: string,
    state: TState
